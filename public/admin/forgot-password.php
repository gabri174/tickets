...
function genericResetMessage(): string {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        } elseif (function_exists('checkRateLimit') && !checkRateLimit('forgot_password_' . $ip, 5, 900)) {
            $message = genericResetMessage();
        } else {
            try {
...
<head>
    ...
    <title>Recuperar Contraseña - <?php echo h(SITE_NAME); ?></title>
    ...
    <div class="glass-card rounded-[2.5rem] p-10 w-full max-w-md relative z-10">
        <div class="text-center mb-10">
            ...
            <p class="text-gray-500 mt-2 font-medium text-xs leading-relaxed px-4">
                Introduce tu correo electrónico y te enviaremos un enlace para restablecer tu clave.
            </p>
        </div>
