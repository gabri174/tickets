...
try {
    ...
    if ($action === 'return') {
        if ($accountId === '') { header('Location: payment_settings.php?stripe=not_started'); exit(); }
        ...
        header('Location: payment_settings.php?stripe=' . urlencode($status)); exit();
    }
    ...
    if ($action === 'onboarding') {
        ...
        header('Location: ' . $link['url']); exit();
    }

    if ($action === 'dashboard') {
        if ($accountId === '') { header('Location: payment_settings.php?stripe=not_started'); exit(); }
        $stripe = new StripeConnectService();
        $link = $stripe->createExpressLoginLink($accountId);
        header('Location: ' . $link['url']); exit();
    }
    ...
    if ($accountId === '') {
        ...
        $email = $admin['email'] ?? '';
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) throw new RuntimeException('El email del organizador no es válido.');
    ...
    qLog('[ERROR] Stripe Connect: ' . $e->getMessage());
    header('Location: payment_settings.php?stripe=error'); exit();
}
